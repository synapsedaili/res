...
def clean_qwen_response(content):
    """Qwen3'ün  etiketlerini ve markdown bloklarını temizler."""
    #  etiketlerini ve içindeki HER ŞEYİ sil
...
{{
    ...
    try:
        ...
        print(f"DEBUG: Raw topic selection response: {content[:500]}...")
        
        clean_content = clean_qwen_response(content)
        print(f"DEBUG: Cleaned content: {clean_content[:300]}...")
        ...
        return {
            ...
            "reason": "Fallback topic due to parsing error",
...
{{
    ...
    try:
        ...
        print(f"DEBUG: Raw evaluation response: {content[:300]}...")
        
        clean_content = clean_qwen_response(content)
        print(f"DEBUG: Cleaned evaluation: {clean_content[:200]}...")
...
{all_data_text}"""
    ...
    try:
        ...
        print(f"DEBUG: Raw report response (first 500 chars): {content[:500]}...")
